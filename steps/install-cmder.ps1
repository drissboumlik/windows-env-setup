...
if ($StepsQuestions["CMDER"].Answer -eq "yes") {
    try {
        ...
        $WhatWasDoneMessages = Set-Error-Message -message "Issue with installing cmder" -exceptionMessage $_.InvocationInfo.PositionMessage -WhatWasDoneMessages $WhatWasDoneMessages
    }
}
#endregion